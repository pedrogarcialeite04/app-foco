...
// Flag de sessão: só sincroniza com o backend depois de ao menos um GET bem-sucedido.
let backendRotinaEnabled = false;
...
export function save(data) {
  ...
  const token = getToken();
  if (token) {
    // Se nesta sessão nunca conseguimos ler do backend, evitamos sobrescrever
    // o que já está salvo lá com um estado potencialmente desatualizado do localStorage.
    if (!backendRotinaEnabled) return;
    const bonus =
