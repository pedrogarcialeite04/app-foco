...
(async function () {
  ...
  // Flag de sessão: só sincroniza metas com o backend se já tiver
  // carregado com sucesso pelo menos uma vez desta API.
  let backendMetasEnabled = false;
  ...
  function saveMetas(metas) {
    ...
    const token = getToken();
    if (!token) return;
    // Se ainda não houve um carregamento bem-sucedido da API nesta sessão,
    // evitamos sobrescrever no backend o que já está salvo com base apenas
    // no estado local (que pode estar vazio ou desatualizado).
    if (!backendMetasEnabled) return;
